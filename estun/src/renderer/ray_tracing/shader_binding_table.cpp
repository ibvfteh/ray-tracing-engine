    ...
    std::vector<uint32_t> programs)
{
    ...
    uint32_t shaderBindingTableSize = shaderBindingTableGroupCount * groupHandleSize;

    buffer_.reset(new estun::Buffer(shaderBindingTableSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT));
    bufferMemory_.reset(new estun::DeviceMemory(buffer_->AllocateMemory(VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT)));

    void *dstData = static_cast<uint8_t *>(bufferMemory_->Map(0, shaderBindingTableSize));
